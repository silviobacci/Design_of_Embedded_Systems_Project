...
TASK(TaskClock) {
	...
	StopWatch2016_step(	&SW_State,	button[0], 		button[1],		button[2],
						button[3], 	button[4], 		button[5],		button[6],
						&hours,		&minutes, 		&seconds,		&tenths,
						&mode, 		&hblink, 		&mblink,		&alarmOn,
						&ringing);
...
int main(void) {
	...
	SW_State.var = &var;
	SW_State.work_mode = &work_mode;

	// Initialize the state machine
	StopWatch2016_initialize(	&SW_State, 	button, 	button+1, 	button+2,
								button+3, 	button+4, 	button+5, 	button+6,
								&hours,		&minutes, 	&seconds, 	&tenths,
								&mode, 		&hblink, 	&mblink,	&alarmOn,
								&ringing);
